...
class IperfManager(object):
    ...
    def start(self, client, time=10, windowsize="16m", packetlen=1400, protocol="udp", tcpdump=False):
        ...
        if tcpdump:
            ...
            if tcpdump == "raw":
                args_tcpdump.extend(["-w", "-"])
            self.client_tcpdump_proc = self.net.get(client).popen(args_tcpdump, stdout=self.tcplogfile)
        else:
            self.tcplogfile = None
            self.client_tcpdump_proc = None
        return client_proc
    ...
    def stop(self):
        ...
        for proc in self.client_procs:
            kill_safe(proc)
        if self.client_tcpdump_proc:
            kill_safe(self.client_tcpdump_proc)
        kill_safe(self.server_proc)
        self.logfile.close()
        self.logfile = None
        if self.tcplogfile is not None:
